...
public  Polynom plus(Polynom other){
        Polynom minP, maxP;
        if (this.getPower() > other.getPower()){ maxP = this; minP = other;
...
public Polynom mul( Polynom other){
        double[] result = new double [this.getPower() + other.getPower()+1] ;
        for(int i = 0; i < this.getPower() +1; i++){
                for(int j = 0; j < other.getPower() +1; j++) {
                         result [i +j] += this.a[i] * other.a[j];
                }
        }
